...
KeyFrame::KeyFrame(const std::string &directory, g2o::HyperGraph *graph)
    : stamp(), odom(Eigen::Isometry3d::Identity()), accum_distance(-1), cloud(nullptr),
      node(nullptr) {
...
void KeyFrame::save(const std::string &directory) {
  ...
  ofs << "stamp " << stamp.sec << "." << std::setw(9) << std::setfill('0') << stamp.nsec
      << "\n";
  ofs.copyfmt(init);
...
bool KeyFrame::load(const std::string &directory, g2o::HyperGraph *graph) {
  ...
  while (!ifs.eof()) {
    ...
    if (token == "stamp") {
      ...
      double fractional_seconds = timestamp - static_cast<double>(stamp.sec);
      stamp.nsec = static_cast<uint32_t>(fractional_seconds * 1e9);
